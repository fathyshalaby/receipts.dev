...
/** Provenance chip — flags self-graded (agent-authored) vs independent claims. */
function sourceChip(source?: string): string {
  const src = source ?? "agent";
  const independent = src !== "agent";
  const title = independent
    ? "Claim authored independently of the implementation."
    : "Claim authored by the agent that did the work — not independently verified.";
  return `<span class="src src--${esc(src)}" title="${esc(title)}">${esc(
...
export function renderReport(m: Manifest): string {
    ...
    .map((r) => {
      ...
      <div class="claim">
        <div class="claim__hd">${badge(r.verdict)}<span class="claim__id">${esc(
          r.acId
        )}</span><span class="claim__text">${esc(r.claim)}</span>${sourceChip(r.source)}</div>
        ${navNote}
        ${adversarialNote}
        ${shots}
        ${framesStrip}
  ...
  const integrity = m.integrity
    ? `<span class="intg" title="manifest ${esc(m.integrity.manifestHash)}">🔒 ${
        m.integrity.signature ? "signed" : "content-hashed"
      } · ${esc(m.integrity.manifestHash.slice(0, 12))} · verify with <code>receipts verify</code></span>`
    : "";
  const footer = `
  <footer class="ft">
